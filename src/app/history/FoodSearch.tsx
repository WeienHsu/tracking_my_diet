    ...
    <section className="flex flex-col gap-3 rounded-xl border border-zinc-200 p-4">
      ...
      <form onSubmit={onSearch} className="flex gap-2">
        <input
          ...
          className="h-12 flex-1 rounded-lg border border-zinc-300 px-3 text-base outline-none focus:border-zinc-500"
        />
        <button
          type="submit"
          disabled={pending || query.trim() === ""}
          className="h-12 shrink-0 rounded-lg bg-black px-5 text-base font-medium text-white disabled:opacity-50"
        >
          {pending ? "…" : "查詢"}
        </button>
      </form>
      ...
      {results != null && (
        <div className="flex flex-col gap-2">
          {results.length === 0 ? (
            <p className="text-sm text-zinc-400">查無紀錄。</p>
          ) : (
            <>
              <p className="text-xs text-zinc-400">{results.length} 筆歷史</p>
